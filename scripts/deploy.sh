...
# ==========================================
# Environment Mapping - Map input to canonical environment
# ==========================================
...
# BACKUP_RETENTION=$(grep "backup_retention_days:" "$CONFIG_FILE" | head -1 | awk '{print $2}')

# ==========================================
# Optional GitHub Actions Environment Variables
# ==========================================
...
# ==========================================
# Step 2: Package Ignition Projects
# ==========================================
echo "Step 2: Packaging Ignition projects..."
...
if [ -d "$PROJECTS_DIR" ] && [ -n "$(ls -A "$PROJECTS_DIR" 2>/dev/null)" ]; then
  ...
  for dir in "$PROJECTS_DIR"/*/; do
    if [ -d "$dir" ]; then
      ...
      echo "  ✓ $project_name packaged"
    fi
  done

else
  echo "No projects found."
  touch "$BUILD_DIR/.no-projects"
fi

echo ""

# ==========================================
# Step 3: Simulated Deployment
# ==========================================
echo "Step 3: Deploying Ignition projects..."

if [ -f "$BUILD_DIR/.no-projects" ]; then

  echo "No projects available for deployment."
  ...
  for project_zip in "$BUILD_DIR"/*.zip; do
    ...
    if [ -f "$project_zip" ]; then
      ...
      echo "  Simulating deployment of $project_name to $ENVIRONMENT"
  ...
  if [ $project_count -eq 0 ]; then
    echo "No packaged projects found."
  fi
fi
...
echo ""
echo "Gateway URL: $GATEWAY_URL"
echo "Open Ignition at:"
echo "${GATEWAY_URL}/web/home"
echo ""
